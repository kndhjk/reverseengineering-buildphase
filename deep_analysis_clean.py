...
# Suppress androguard debug logging
logging.getLogger('androguard').setLevel(logging.CRITICAL)
for name in ['androguard.misc', 'androguard.core.apk', 'androguard.core.axml',
             'androguard.core.dex', 'androguard.core.analysis.analysis',
...
def analyze_apk_deep(apk_path, name):
    ...
    with zipfile.ZipFile(apk_path, 'r') as zf:
        ...
        print(f"  Found {len(so_files)} .so files: {[os.path.basename(s) for s in so_files]}")
    ...
    # 8. Look for network/endpoint URLs in DEX strings
    print(f"\n--- NETWORK/ENDPOINT STRINGS ---")
    for dex in d:
        for s in dex.get_strings():
            if 'http' in s.lower() and '.' in s and len(s) < 500:
                print(f"  {s[:300]}")

    # 9. Look for interesting class/method names related to crypto/auth
    print(f"\n--- CRYPTO/AUTH RELATED CLASSES ---")
    for dex in d:
        for cls in dex.get_classes():
            cls_name = cls.get_name()
            if any(kw in cls_name.lower() for kw in ['crypto', 'auth', 'key', 'token', 'secret', 'cipher', 'encrypt', 'decrypt', 'sign', 'verify']):
                print(f"  {cls_name}")
